...
class ProductShow extends Json
{
    ...
    function build()
    {
        ...
        $comments = new CommentsPublicList($this->request);
...
//        'item_video' => 'product/show/item_video.tpl',
        ...
        $this->tpl->assign(array(
            ...
            'ITEM_PRICE' => reductionToCurrency($this->item['price']),
        ...
        foreach ($tabs as $tabkey => $tabcfg) {
            ...
            if (isset($tabcfg['title'])) {
                $tabtitle = Lang::get($tabcfg['title']);
        ...
        //promos
        $_promo = _oh('promotion');
        $qm_promo = new QueryMaker($_promo, false, true);
    ...
    {
        $_product = _oh($this->request->ot_id);
        $cur = _mod('cart')->getCurrency();

        $variants = array(
            $this->request->iid => array(
                'size' => $item['size'],
                'price' => reductionToCurrency($item['price'] * $cur->rate),
        ...
        if (!empty($item['_variants'])) {
            foreach ($item['_variants'] as $v) {
                $variants[$v[$_product->getPAC()]] = array(
                    'price' => reductionToCurrency($v['price'] * $cur->rate),
                    'color' => $v['color'],
                    'size' => $v['size'],
            ...
            uasort($variants, array(_mod('product'), 'sortVariants'));
        }
        ...
        foreach ($variants as $vid => $cvar) {
            ...
            $this->tpl->assign(array(
                ...
                'VARIANT_IN_STOCK_TITLE' => Lang::get('products fields in_stock ' . ((int)$cvar_in_stock)),
