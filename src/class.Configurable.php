...
class Configurable extends Eventer {
  ...
  protected function _findRealMethod($method){
    return;
  }
  ...
  */
  ...
  function applyConfig($cfgNamesArr, $params = array()){
    ...
    if(!reductionToArray($cfgNamesArr, ' ')){
      return false;
    }
    foreach($cfgNamesArr as $cconfName){
      ...
      $params['path'] = hive::getRealIncludeLocation($cconfigFilePath);
  ...
  public function getPublicVars() {
    return get_object_vars_public($this);
  }
  ...
  private function applyChangeToProps($cfg){
    ...
    foreach($cfg as $k => $v){
      ...
      if(method_exists($this, $mtd)){
        $this->$mtd($v);
      }elseif(is_array($realWorker = $this->_findRealMethod($mtd))){
        $realWorker[0]->{$realWorker[1]}($v);
      }else{
        ...
        // Если значение объект ConfigurablePropRestore
        ...
        if(is_object($v) && $v instanceof ConfigurablePropRestore){
          $p = $v->run($this, $k);
...
class ConfigurableProp{

}

class ConfigurablePropRestore{
